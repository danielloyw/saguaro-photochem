...
def plot_molecule(srun,smolec,pflag):

    RT = 2575.E5
    colrates_lim = 1.0E-03
...
#    ext_prd = read_ext('../runs/'+srun+'/input/oxy_input.dat',smolec)
    alt = crates["alt"]
    nalt = len(alt)
...
#    rct = np.concatenate((prct,erct,grct,crct),axis=1)
    ...
    if (nprd > 0):
      ...
      sort_indx_prd = np.argsort(ftmp_prd)[::-1]
      ...
      for n in xrange(1, nprd):
        lfind = np.bool_(1)
        m = 0
        while np.logical_and(np.less_equal(m,k),(lfind)):       
          ...
          if np.logical_and((sp2[n] != ''),(sp3[m] != '-')):
              lchk = np.logical_and(lchk,l4)
          if np.logical_and((sp3[n] != ''),(sp3[m] != '-')):
              lchk = np.logical_and(lchk,l5)        
          if(lchk):
              lfind = np.bool_(0)
    ...
    if (nlss > 0):
      ...
      sort_indx_lss = np.argsort(ftmp_lss)[::-1]
    ...
    fig = plt.figure()    
    fig.suptitle(srun+': '+smolec+' SUMMARY')
    
    # plot density upper left
    
    ax1 = fig.add_subplot(2, 2, 1)
    xmax = np.power(10.,int(np.log10(np.amax(molsum["den"])))+1)
    xmin = xmax/1.E10
    ...
    xmax = np.power(10.,int(np.log10(np.amax(molsum["mol"])))+1)
    xmin = xmax/1.E10
    ...
    if(xmax > 1.E-10):
        xmax = np.power(10.,int(np.log10(xmax))+1)
        xmin = xmax/1.E10        
        ...
        ax3.semilogx([3*xmin,30*xmin],[ymax-20,ymax-20],color=clrs[0])
        ax3.text(50*xmin,ymax-20,r'Net Chemical Production',color=clrs[0],fontsize='6')                
        ax3.semilogx([3*xmin,30*xmin],[ymax-35,ymax-35],color=clrs[1])
        ax3.text(50*xmin,ymax-35,r'Net Chemical Loss',color=clrs[1],fontsize='6')       
        ax3.semilogx([3*xmin,30*xmin],[ymax-50,ymax-50],color=clrs[2])         
        ax3.text(50*xmin,ymax-50,r'Flux Divergence',color=clrs[2],fontsize='6')                
        ax3.semilogx([3*xmin,30*xmin],[ymax-65,ymax-65],color=clrs[2],linestyle=':')         
        ax3.text(50*xmin,ymax-65,r'Flux Convergence',color=clrs[2],fontsize='6')                
    ...
    if(np.amax(abs(flx))>1.E-10):
        ...
        if (np.size(dnflx)>0):
            dmax = np.amax(dnflx)
        if((np.size(upflx)>0) & (np.size(dnflx)>0)):            
            xmax = np.amax([umax,dmax])
        elif((np.size(upflx)>0) & (np.size(dnflx==0))):
            xmax = np.amax(umax)
        elif((np.size(upflx)==0) & (np.size(dnflx>0))):
            xmax = np.amax(dmax)            
        xmax = np.power(10,int(np.log10(xmax))+1)
        ...
        ax4.semilogx([3*xmin,30*xmin],[ymax-20,ymax-20],color=clrs[0])         
        ax4.text(50*xmin,ymax-20,r'Upward Flux',color=clrs[0],fontsize='6')                
        ax4.semilogx([3*xmin,30*xmin],[ymax-40,ymax-40],color=clrs[0],linestyle=':')         
        ax4.text(50*xmin,ymax-40,r'Downward Flux',color=clrs[0],fontsize='6')                
    ...
    if(nprd>0):
        ...
        fig = plt.figure()      
        fig.suptitle(srun+': '+smolec+' PRODUCTION')
        
        # upper left

#        ext_max = np.amax(ext_prd)
        l1 = l 
        l2 = max([l1,min([l + 5,nprd-1])])
        if (l1 == l2):
            xmax = np.amax(rates_prd[:,l1])
#            xmax = math.pow(10.,int(math.log10(np.amax([rates_prd[:,l1],ext_max]))))
        else:    
            xmax = np.amax(rates_prd[:,l1:l2+1])
#            xmax = math.pow(10.,int(math.log10(np.amax([rates_prd[:,l1:l2+1],ext_max]))))

#        xmax = np.amax([xmax,ext_max])
#        xmax = math.pow(10.,int(math.log10(np.amax([rates_prd[:,l1:l2+1],ext_max]))))
        xmax = np.power(10.,int(np.log10(xmax))+1)
...
#            xmax = math.pow(10.,int(math.log10(np.amax(rates_prd[:,l1:l2+1])))+1)
 ...
 #       if(ext_max>0):           
 #           k=0
 #           ax1.semilogx(ext_prd,alt,color=clrs[k])
 #           ax1.text(3*xmin,ymax-100-k*60,r'External Input',color=clrs[k],fontsize='6')
 #           k = k + 1
        
        while ((k<5) & (l<nprd)):
            ax1.semilogx(rates_prd[:,l],alt,color=clrs[k])
            ax1.text(3*xmin,ymax-15-k*12,title_prd[l],color=clrs[k],fontsize='6')
            l = l + 1
            k = k + 1
        ...
        if(l<nprd):
            ...
            l2 = max([l1,min([l + 5,nprd-1])])
            if (l1 == l2):
                xmax = np.power(10.,int(np.log10(np.amax(rates_prd[:,l1])))+1)
            else:    
            ...
            while ((k<5) & (l<nprd)):
                ...
                ax2.text(3*xmin,ymax-15-k*12,title_prd[l],color=clrs[k],fontsize='6')
                l = l + 1
                k = k + 1
        ...
        if(l<nprd):
            ...
            while ((k<5) & (l<nprd)):
                ...
                ax3.text(3*xmin,ymax-15-k*12,title_prd[l],color=clrs[k],fontsize='6')
                l = l + 1
                k = k + 1
        ...
        if(l<nprd):
            ...
            while ((k<5) & (l<nprd)):
                ...
                ax4.text(3*xmin,ymax-15-k*12,title_prd[l],color=clrs[k],fontsize='6')
                l = l + 1
                k = k + 1
    ...
    if(nlss>0):
        ...
        fig.suptitle(srun+': '+smolec+' LOSS')
        
        # Upper left

        cdn_max = np.amax(rcdn)
        l1 = l 
        l2 = max([l1,min([l + 5,nlss-1])])
        if (l1 == l2):
            xmax = np.amax(rates_lss[:,l1])
#            xmax = math.pow(10.,int(math.log10(np.amax([rates_prd[:,l1],ext_max]))))
        else:    
            xmax = np.amax(rates_lss[:,l1:l2+1])
#            xmax = math.pow(10.,int(math.log10(np.amax([rates_prd[:,l1:l2+1],ext_max]))))

        xmax = np.amax([xmax,cdn_max])
#        xmax = math.pow(10.,int(math.log10(np.amax([rates_prd[:,l1:l2+1],ext_max]))))
        xmax = np.power(10.,int(np.log10(xmax))+1)
...
#            xmax = math.pow(10.,int(math.log10(np.amax(rates_lss[:,l1:l2+1])))+1)
        ...
        if(cdn_max>0):           
            k=0
            ax1.semilogx(rcdn,alt,color=clrs[k])
            ax1.text(3*xmin,ymax-15-k*12,r'Condensation',color=clrs[k],fontsize='6')
            k = k + 1
                
        while ((k<5) & (l<nlss)):
            ax1.semilogx(rates_lss[:,l],alt,color=clrs[k])
            ax1.text(3*xmin,ymax-15-k*12,title_lss[l],color=clrs[k],fontsize='6')
            l = l + 1
            k = k + 1
        ...
        if(l<nlss):
            ...
            l2 = max([l1,min([l + 5,nlss-1])])
            if (l1 == l2):
                xmax = np.power(10.,int(np.log10(np.amax(rates_lss[:,l1])))+1)
            else:                 
            ...
            while ((k<5) & (l<nlss)):
                ...
                ax2.text(3*xmin,ymax-15-k*12,title_lss[l],color=clrs[k],fontsize='6')
                l = l + 1
                k = k + 1
        ...
        if(l<nlss):    
            ...
            while ((k<5) & (l<nlss)):
                ...
                ax3.text(3*xmin,ymax-15-k*12,title_lss[l],color=clrs[k],fontsize='6')
                l = l + 1
                k = k + 1
        ...
        if(l<nlss):
            ...
            while ((k<5) & (l<nlss)):
                ...
                ax4.text(3*xmin,ymax-15-k*12,title_lss[l],color=clrs[k],fontsize='6')
                l = l + 1
                k = k + 1
