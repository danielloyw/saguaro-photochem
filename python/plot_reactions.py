...
#pflag = np.int(sflag)
pflag = 1
splotfile = '../runs/'+srun+'/plots/reaction_rates.pdf'
...
rctmax = np.zeros(nrct)
for l in range(0,nrct):
    rctmax[l] = np.amax(rct[:,l])

rct_sav = rct
rctmax_sav = rctmax
sort_indx = np.argsort(rctmax)[::-1]
rct_tmp = rct
rctmax = rctmax[sort_indx]
title_tmp = title
for l in range(0,nrct):
    j = sort_indx[l]
    rct[:,l] = rct_tmp[:,j]
    title[l] = title_tmp[j]
    print l, j, np.amax(rct[:,l])
#    rctmax[l] = rctmax_tmp[j]
...
#        print title[l]
    ...
    if (l == nrct-1): 
        ax1.set_xlabel(r'Reaction Rate (cm$^{-3}$s$^{-1}$)',fontsize='8')                          
...
#            print title[l]
            ...
            if (l == nrct-1): 
                ax1.set_xlabel(r'Reaction Rate (cm$^{-3}$s$^{-1}$)',fontsize='8')                 
                ax2.set_xlabel(r'Reaction Rate (cm$^{-3}$s$^{-1}$)',fontsize='8')                      
...
#            print title[l]
        ...
        ax3.set_xlabel(r'Reaction Rate (cm$^{-3}$s$^{-1}$)',fontsize='8')
...
#            print title[l]
        ...
        ax4.set_xlabel(r'Reaction Rate (cm$^{-3}$s$^{-1}$)',fontsize='8')
