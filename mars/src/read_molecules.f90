SUBROUTINE READ_MOLECULES
  ...
  READ(61,*) neutrmax ! number of neutral species
  READ(62,*) nionsmax ! number of ion species

  nsp = neutrmax + nionsmax + 1 ! total number of species + electrons
...
961 FORMAT(I4,1X,A12,1X,I1,1X,I2,1X,F7.1,5I4,2X,I4,1X,ES9.2,1X,F6.3,1X,ES9.2,1X,                    &
  ...
  READ(62,"(A)") header
  ichk = 0 ! number of ions in chemical eqm
  DO n = 1, nionsmax
     nm = neutrmax + n
...
962 FORMAT(I4,1X,A12,1X,I1,1X,I2,1X,F7.1,5I4,2(I3,ES11.3))
  ...
  nx = 0
  DO nm = 1, nsp-1
     IF(istat(nm) == 1) THEN ! species in chem equilibrium
        nx=nx+1
        locp(nx) = nm
        lopc(nm) = nx
     END IF
  ...
  END IF
!!!
!For Mars:  CO2 (0.95), N2(0.027), Ar40(0.016)?
!!!
  iN2 =  FIND_NAME('N2          ',name)
  iCO2 =  FIND_NAME('CO2         ',name)
