PROGRAM PCHEM1D
  ...
  INTEGER :: ntim_diff, ntim_chem, iter_chem, iter_diff
  INTEGER :: nw, na, nm, nl, n, nz, nx, ne, np, nb, nln, nmn, imax(2), nm1, nm2, nm3, nm4, nm5
  INTEGER :: isol, iprnt
  REAL(RP) :: dnm, sm, tst1
  ...
  nreact_file = 'nreactions.csv'
  ireact_file = 'ireactions.csv'
  treact_file = 'nreactions.tab'
...
!  READ(*,*) tstep_diff, ntim_diff
  ...
  CALL ELCTRN                   !  Set up suprathermal electron calcualtion

  CALL READ_AEROSOL             !  Read Aerosol Model

  CALL READ_RAYLEIGH            !  Calculate Rayleigh optical depth

  CALL PATHS1                   !  Calculate path lengths for solar absorption
...
!                    WRITE(*,"(2I4,A12,5ES11.3)") nln,nmn,name(nmn),pr(nln,nmn),ls(nln,nmn),test_chem
     ...
     lcrsA = .false.; lcrsB = .false.; lcrsC = .true.; lcrsJ = .true.
     CALL COMPO 
     ...
     DO nm = 1, nsp
        DO nl = 1, nlev
           IF( ((istat(nm) == 2).and.(den(nl,nm)/den(nl,0) > 1.E-12_RP)) .or.                       &
              ...
              IF(dnm > 1.E-12_RP) THEN
                 tst1 = ABS(dNdt(nl,nm))/dnm
              ELSE
                 tst1 = zero
              END IF
     ...
     WRITE(*,911) iter_diff,name(nmn), dNdt(nln,nmn), pr(nln,nmn), ls(nln,nmn),   &
          div_flx(nln,nmn), 1.E-5*z(nln), den(nln,nmn)/den(nln,0)
     
     !  .. Write to Disc
     
     IF (MOD(iter_diff-1,iprnt) == 0) CALL COMPOUT
...
911 FORMAT(5X,' STEP=',I6,2X,A12,' DNDT = ',ES11.3,' Pr = ',ES11.3,' Ls = ',   &
      ES11.3,' div_flx = ',ES11.3,' Z = ',ES12.4,' X = ',ES11.3)
