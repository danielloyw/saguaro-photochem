SUBROUTINE READ_ATMOS
  ...
  OPEN (unit=63,file='atm1D.in',status='old',action='read')
     ...
     ALLOCATE(namex(nmol),indx_den(nmol),z(nlev),rz(nlev),grv(nlev),ek(nlev),tn(nlev),ti(nlev),     &
          te(nlev),prs(nlev),mass(nlev),rho(nlev),den(nlev,0:nsp),den_old(nlev,0:nsp))
  ...
  nm = FIND_NAME('CO          ',name)
  den(1:nlev,nm) = 5.E-5_RP*den(1:nlev,0)
 
  !  ..  Set neutral density = min val if species not found
  ...
  DO nz = 1, nlev
     den(nz,0) = SUM(den(nz,1:nsp-1))
  END DO

  WHERE (den < DMIN) den=DMIN

  !  .. Reset total density to be consistent with P & T

  DO nz = 1, nlev
     den(nz,0) = prs(nz)/rkb/tn(nz)
  END DO
  ...
  DO nz = 1, nlev
     IF (nionsmax > 0) THEN
        ...
        mass(nz) = rho(nz)/(den(nz,0)+two*den(nz,nsp))/amu
     ELSE
        rho(nz) = amu*DOT_PRODUCT(mmw(1:nsp),den(nz,1:nsp))
