SUBROUTINE DIFCO
  ...
  REAL(RP) :: dN2, dCH4, mN2, mCH4, rmN2, nuN2, bN2, rmCH4, nuCH4, bCH4, sigma, mu
  INTEGER :: nl, nx, nm
  ...
  DO nx = 1, ndiff
     nm = ldcp(nx)
     IF (dtype(nm) == 0) THEN
        df(nl,nm) = ad(nm)*(tn(nl)**sd(nm))/den(nl,0)
     ELSE IF (dtype(nm) == 1) THEN
        df(nl,nm) = ad(nm)*(tn(nl)**sd(nm))*EXP(-sd_2(nm)/tn(nl))/den(nl,0)
     ELSE IF (dtype(nm) == 2) THEN
        df(nl,nm) = (ad(nm)*(tn(nl)**sd(nm))*EXP(-sd_2(nm)/tn(nl))            &
             *EXP(-sd_3(nm)/tn(nl)**2))/(den(nl,0)*(LOG(phi(nm)/tn(nl)))**2)
     ELSE IF (dtype(nm) == 3) THEN                                               ! hard sphere approximation
        sigma = 3.0E-15_RP
        mu = amu/(one/mmw(nm) + one/mmw(iN2))
        df(nl,nm) = (two/three/sqpi)*(one/sigma)*SQRT(rkb*tn(nl)/mu)/den(nl,0)
     ELSE                                                                        !  Need to add Lennard-Jones approximation
...
!           bval(nm,2) = WJEANS (mmw(nm), rz(nlev), tn(nlev))
           bval(nm,2) = two*WJEANS (mmw(nm), rz(nlev), tn(nlev))
     END IF
  ...
  DO nx = 1, ndiff
     ...
     DO nl =1, nlev-1
        ...
        beta(nl,nx) = dfp(nl,nm)/htp(nl,nm) + ekp(nl)/htp(nl,0)            &
  ...
  DO nx = 1, ndiff
     ...
     DO nl = 2, nlev-1                  
        a(nl,nx) = -(alpha(nl-1,nx)-half*beta(nl-1,nx))*rm2(nl)/dr(nl)
        b(nl,nx) = ((alpha(nl,nx)-half*beta(nl,nx))*rp2(nl)/dr(nl)         &
             + (alpha(nl-1,nx)+half*beta(nl-1,nx))*rm2(nl)/dr(nl))
        c(nl,nx) = -(alpha(nl,nx)+half*beta(nl,nx))*rp2(nl)/dr(nl)
     END DO
     ...
     ELSE IF (ibnd(nm,1) == 2) THEN              !  .. Fixed Velocity        
        b(nl,nx) = -(-alpha(nl,nx)+half*bval(nm,1)                           &
             +half*beta(nl,nx))/drp(nl)
        c(nl,nx) = -(alpha(nl,nx)+half*bval(nm,1)                            &
             +half*beta(nl,nx))/drp(nl)
     ELSE IF (ibnd(nm,1) == 3) THEN              !  .. Fixed Mole Fraction
        b(nl,nx) = zero           
        c(nl,nx) = zero
     ELSE
        WRITE(*,*) ' TCHEM: ERROR IN LOWER B.C., EXITING ...'
        STOP
     END IF
     ...
     IF(ibnd(nm,2) == 1) THEN                     !  .. Chemical Equilibrium
        a(nl,nx) = zero
        b(nl,nx) = zero
     ELSE IF(ibnd(nm,2) == 2) THEN                !  .. Fixed Velocity
        a(nl,nx) = -(alpha(nl-1,nx)                                         &
             -half*beta(nl-1,nx)-half*bval(nm,2))*rm2(nl)/drp(nl-1)
        b(nl,nx) = (half*bval(nm,2)+alpha(nl-1,nx)                          &
             +half*beta(nl-1,nx))*rm2(nl)/drp(nl-1)
     ELSE
        WRITE(*,*) ' TCHEM: ERROR IN UPPER B.C., EXITING ...'
        STOP
     END IF
